...
### Известные предупреждения ESLint

При установке зависимостей через npm могут появляться предупреждения:

1. **`@typescript-eslint/no-unused-vars`** - отключено в конфигурации
2. **`error: any`** в catch блоках - стандартная практика TypeScript
3. **Deprecation warnings** в зависимостях - не влияют на работу приложения

### Безопасность

Система использует следующие меры безопасности:

1. **Безопасный калькулятор** - собственный парсер математических выражений без `eval()` или `new Function()`
2. **Санитизация данных** - очистка HTML в веб-поиске
3. **CORS-прокси** - только для публичных данных
4. **Локальное хранение** - все данные остаются на устройстве пользователя
5. **Нет серверного кода** - полностью клиентское приложение
...
### Типичные предупреждения npm audit

Большинство предупреждений относятся к:
- **Dev dependencies** (Vite, ESLint) - не влияют на продакшн
- **Транзитивные зависимости** - исправляются при обновлении основных пакетов
- **Низкий приоритет** - не представляют реальной угрозы для клиентского приложения

### Рекомендации

1. Использовать `bun install` вместо `npm install` для меньшего количества предупреждений
2. Регулярно обновлять зависимости через Lovable UI
3. При критических уязвимостях - обновлять конкретные пакеты
